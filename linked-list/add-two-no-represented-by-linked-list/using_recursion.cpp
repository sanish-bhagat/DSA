...
Node *addRec(Node *first, Node *second, int &carry)
{
    ...
    //recursively get the sum of the remaining nodes
    result ->next = addRec(first, second, carry);
...
Node *addTwoLists(Node *first, Node *second)
{
    ...
    Node *result = addRec(first, second, carry);

    //if any carry is present make a new node of it
    if(carry != 0) {
